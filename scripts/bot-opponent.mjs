...
mm.addEventListener("open", () => mm.send(JSON.stringify({ kind: "queue", modality: MODALITY, name: NAME })));
...
function playBattle(battleId, role) {
	...
	const sent = new Set();
	ws.addEventListener("open", () => ws.send(JSON.stringify({ kind: "hello", role, name: NAME })));
	ws.addEventListener("message", (ev) => {
		...
		if (s.phase === "turn" && s.activeRole === role && !sent.has(s.round)) {
			...
			const verse = `Soy ${NAME} y en la ronda ${s.round} te paso por encima con flow`;
